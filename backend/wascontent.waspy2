...
class ContentProcessor:
    ...
    def process_existing_postqq(self, content: str) -> str:
        ...
        # Check if this looks like it contains an image markdown reference
        if '![' in content and '](' in content:
            # Replace any markdown image references that point to our API
            img_pattern = r'!\[([^\]]*)\]\((/forum/api/image/\d+)\)'
            
            def enhance_image_tag(match):
                alt_text, img_url = match.groups()
                # Create a more explicit markdown image reference
                return f'![{alt_text}]({img_url})'
            
            content = re.sub(img_pattern, enhance_image_tag, content)
        
        # Convert markdown to HTML
        html = self.md.convert(content)
        
        # Clean the HTML while preserving our allowed tags and attributes
        clean_html = bleach.clean(
            html,
            tags=self.allowed_tags,
            attributes=self.allowed_attrs,
