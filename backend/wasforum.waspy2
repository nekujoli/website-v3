...
def view_thread(thread_id: int):
    ...
    with get_db() as conn:
        ...
        for post in original_posts:
            ...
            post_dict = dict(post)
            # Process the content for Markdown rendering
            post_dict['content'] = processor.process_existing_post(post_dict['content'])
...
def post_history(post_id: int):
    ...
    with get_db() as conn:
        ...
        for edit in edits:
            edit_dict = dict(edit)
            edit_dict['old_content'] = processor.process_existing_post(edit_dict['old_content'])
            edit_dict['new_content'] = processor.process_existing_post(edit_dict['new_content'])
